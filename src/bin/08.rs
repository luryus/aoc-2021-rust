...
fn deduce(line: Vec<&str>) -> Option<SevenSegment> {
    ...
    let (top_right, bottom_right) = one_segs
        .sorted_by_key(|c| line.iter().filter(|x| x.chars().contains(c)).count())
        .collect_tuple()?;
    let top = line
        .iter()
        .find(|x| x.len() == 3)?
        .chars()
        .find(|&c| c != top_right && c != bottom_right)?;
    let four_left_segs = line
        .iter()
        .find(|x| x.len() == 4)?
        .chars()
        .filter(|&c| c != top_right && c != bottom_right);
    let (top_left, middle) = four_left_segs
        .sorted_by_key(|c| line.iter().filter(|x| x.chars().contains(c)).count())
        .collect_tuple()?;
    let bot_left_bot = line
        .iter()
        .filter(|x| x.len() == 5)
        .map(|x| x.chars())
        .flatten()
